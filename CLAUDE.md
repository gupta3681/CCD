...
│   ├── userSettings.ts       # reads/writes ~/.claude/CLAUDE.md and ~/.claude/skills/<name>/SKILL.md
...
Loaded `.claude/` sources are controlled per-conversation:
- Always loads `'user'` (`~/.claude/`) — the user's global skills + CLAUDE.md.
- Loads `'project'` (the cwd's `.claude/`) **only if** the conversation's `trustProject` flag is true. Toggled by the "Trust this folder's `.claude/`" checkbox in the right sidebar. Auto-resets when cwd changes.

This is the prompt-injection defense: opening an untrusted repo can't auto-load that repo's skills or `CLAUDE.md` into the agent.
...
| `~/.claude/projects/<cwd-hash>/<session-id>.jsonl` | SDK | required for `resume: sessionId` to work — multi-turn memory |
...
- **As a Claude Code skill**: drop a `SKILL.md` (with frontmatter) into `resources/bundled/skills/<name>/`. To load it, set `cwd` to a workspace dir that includes it OR pass via `settingSources` / `skills`.
...
- Default model: `claude-sonnet-4-6` (override `PORTICO_MODEL`)
...
- SDK session JSONL: `~/.claude/projects/<sanitized-cwd>/<session-id>.jsonl`
